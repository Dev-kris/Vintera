...
async function buildCartHTML() {
  for (var i = 0; i < localStorage.length; i++) {
    ...
    document
      .getElementById("cart-camera-details" + [i])
      .appendChild(tableDataDivImg);
    document
      .getElementById("cart-camera-details" + [i])
      .appendChild(tableDataDivDetails);
...
async function getCameraDetails() {
  for (var i = 0; i < localStorage.length; i++) {
    ...
    document.getElementById("camera-subtotal" + [i]).innerHTML =
      "$ " + lineSubtotal;
  }
  await getCartTotals();
}
async function getCartTotals() {
  ...
  let cartTax = cartSubtotal * 0.2;
  var cartTotal = cartSubtotal + cartTax;

  document.getElementById("subtotal-data").innerHTML =
